...
async def list_models(
    ...
    """List available LLM models from provider APIs (live, no hardcoded lists).

    Calls each provider's REST API directly so retired models are automatically
    excluded. Falls back to litellm static registry on API failure.
